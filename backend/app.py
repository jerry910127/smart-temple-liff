...
def get_casual_fallback(user_text: str) -> str:
    """智能情境客觀保底庫（依據問題語境給予實質引導）"""
    t = user_text.lower()
    if any(k in t for k in ["神明", "廟", "宮", "保佑", "拜"]):
        return (
            "信士您好，關於您詢問的宮廟神明與信仰事宜：傳統宮廟敬奉神明以「心誠則靈」為上，"
            "若欲查詢特定宮廟歷史（如台中萬春宮湄洲媽祖）或參拜儀軌，歡迎隨時詢問，亦可點選下方「參拜足跡」探索名廟文化。"
        )
    elif any(k in t for k in ["籤", "運勢", "未來", "事業", "感情", "功名"]):
        return (
            "信士您好，若欲請示近期運勢或指引迷津，建議可前往「湄洲媽靈籤」專區誠心搖筒請示神明：\n"
            "👉 https://liff.line.me/2011668576-3Qay1nBv?page=divination\n"
            "獲賜籤詩後回傳此處，線上服務處將立即為您客觀解析。"
        )
    return (
        "信士您好，訊息已收到。智慧宮廟線上服務處隨時為您提供參拜儀軌、線上靈籤求請、祈安點燈與正念冥想指引。"
