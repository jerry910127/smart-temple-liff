...
app = FastAPI(title="靈籤入微 - AI 智慧宮廟後端 Webhook", version="2.0.0")
...
def get_ai_interpretation(user_message: str) -> str:
    ...
    try:
        ...
        print(f"NVIDIA API 調用失敗: {e}", file=sys.stderr)
        return f"【老廟祝合十】善信莫慌，神意常在。老廟祝剛才閉目參祥天機，暫時連線稍有延遲。請稍候再向神明請教一次。\n（錯誤提示：{str(e)[:50]}）"
    ...
    with ApiClient(configuration) as api_client:
        ...
        for event in events:
            if isinstance(event, MessageEvent) and isinstance(event.message, TextMessageContent):
                ...
                # 顯示 LINE 原生「輸入中 (Loading Animation)」提示 (加強使用者體驗)
                try:
                    if user_id:
                ...
                # 判斷是否為求籤或解籤訊息
                if any(kw in user_text for kw in ["靈籤", "籤", "聖杯", "解惑", "求籤", "老廟祝", "神明"]):
                    ...
                    reply_text = (
                        ...
                        "抽得靈籤後，老廟祝會運用神明啟發與人生經驗，為您溫暖剖析指引迷津！"
                    )
