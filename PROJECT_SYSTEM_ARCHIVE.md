...
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `LINE Bye` 品牌標題 ($X \approx 5\% \sim 30\%, Y \approx 2\% \sim 7\%$)
...
* **動態視覺化足跡地圖區 (Visual Track: $Y \approx 52\% \sim 95\%$)**：
  * 步道軌跡容器：向量 SVG 蜿蜒虛線路徑
  * 腳印 1（左腳）：實線/已解鎖狀態（金色發光）
  * 虛線路徑 1：由腳印 1 延伸至腳印 2
  * 腳印 2（右腳）：半透明/待感應狀態（感應後轉為金色）
  * 虛線路徑 2：由腳印 2 延伸至腳印 3
  * 腳印 3（左腳）：半透明/待感應狀態
* **LINE Beacon / 定位感應業務邏輯**：
  * GPS 即時測距：透過 `navigator.geolocation` 計算與廟宇直線距離
  * 當距離 $\le 50\text{m}$ 或點擊「📡 現場打卡」/「模擬 Beacon 打卡」時：
    1. 站點代號圓圈切換為打勾 `✓`
    2. 對應腳印點亮轉為綠色實線發光
