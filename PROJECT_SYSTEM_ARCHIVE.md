...
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `LINE BYE` 品牌標題 ($X \approx 5\% \sim 30\%, Y \approx 2\% \sim 7\%$)：`LINE` 為 `#2B1810`（沉穩檀木黑，1.35rem 粗體）；`BYE` 採用 `#7A0C16`（深緋紅，1.35rem 粗體，與 LINE 完全等大等重）；緊鄰右側點綴**「一對朱紅神光筊（雙筊聖杯造型）」**向量微動態裝飾（成雙成對微帶金色鑲邊與靈動光暈，點擊互動浮動），右側並搭配香檳金棕 `MINI` 徽章，完美融入 LINE Pay / LINE GO 設計語言與傳統宮廟神聖意象
