...
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `LINE BYE` 品牌標題 ($X \approx 5\% \sim 30\%, Y \approx 2\% \sim 7\%$)：`LINE` 為 `#2B1810`（沉穩檀木黑，亦可切換 `#7A0C16` 深緋紅）；`BYE` 為 `#C99F72` 古法香檳金棕膠囊底牌（帶有金漆牌匾溫潤光澤）搭配 `#7A0C16` 深緋紅文字；右側搭配金棕光澤 `MINI` 徽章
