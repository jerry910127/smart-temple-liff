...
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `LINE BYE` 品牌標題 ($X \approx 5\% \sim 30\%, Y \approx 2\% \sim 7\%$)：`LINE` 為 `#2B1810`（沉穩檀木黑，1.35rem 粗體）；`BYE` 採用宮廟正統**「朱紅神光月牙聖杯（筊）」**向量底牌（朱砂赤木漸層搭配朱紅立體光暈），上方鐫刻等大 `1.35rem` 溫潤金箔白文字（`#FFF6E5`），字體大小與 LINE 完全等比對齊；右側搭配朱紅神光 `MINI` 徽章
