...
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `LINE BYE` 品牌標題 ($X \approx 5\% \sim 30\%, Y \approx 2\% \sim 7\%$)：`LINE` 為 `#2B1810`（沉穩檀木黑，1.32rem 粗體）；`BYE` 採用「聖筊圓弧半月造型」與「微笑嘴唇」結合之手繪向量底牌——外層基底為朱砂暗紅 `#981617`，上方開口凹槽填入宮廟深緋紅 `#6B0A0D` 營造立體景深，中央刻劃典雅金棕 `#C68F42` 的 `BYE` 文字，下緣點綴豆沙肉粉 `#C97779` 微笑月牙高光（85% 透明度）；右側搭配典雅金棕透光 `MINI` 徽章，融合傳統文化意象與現代 App 親切視覺
