...
### 1. 頁面一：首頁（LINE Bye 主入口）
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `LINE BYE` 品牌標題 ($X \approx 5\% \sim 30\%, Y \approx 2\% \sim 7\%$)：`LINE` 為 `#2B1810`（沉穩檀木黑，1.35rem 粗體）；`BYE` 採用「聖筊圓弧半月造型」與「微笑嘴唇」結合之手繪向量底座（朱砂暗紅 `#981617` 外層、深緋紅 `#6B0A0D` 凹槽、豆沙肉粉 `#C97779` 微笑月牙光），中央以 HTML 獨立渲染**典雅金棕 `#C68F42` 的 `BYE` 文字（1.35rem 粗體，與 LINE 完全等大等重）**；右側搭配典雅金棕透光 `MINI` 徽章，兼具文化神韻與現代品牌高識別度
  * 右側 4 圖示：優惠券（周邊商圈核銷，55%~70%）、說明 `?` (72%~78%)、即時消息鈴鐺 (80%~87%)、設定齒輪 (88%~95%)
* **主題推薦橫幅 (Hero Banner: $Y \approx 8\% \sim 32\%$)**：
  * 脫單專區文案 + 右側手持立香與月老紅線 SVG 插畫
* **四大核心服務入口 (Quick Access Grid: $Y \approx 32\% \sim 45\%$)**：
  * `線上解籤`（串 Web 抽籤擲筊）、`點燈專區`（合作宮廟在線代辦表單）、`參拜足跡`（**點擊直通頁面二**）、`節慶專區`（繞境與燈謎）
* **附近廟宇探索區 (Near Temples: $Y \approx 45\% \sim 75\%$)**：
  * 龍山寺、城隍廟、行天宮卡片，標註 `📡 BT 微型感應`
* **底部跑馬燈橫條 (Footer Tips: $Y \approx 75\% \sim 83\%$)**：
  * 輪播新手參拜小撇步（右手插香等禮節）
* **底部付費廣告版位 (Ad Banner: $Y \approx 83\% \sim 92\%$)**：
  * 標註「付費廣告」，提供宮廟年度法會或伴手禮商家商業投放
* **底部導航欄 (Bottom Dock: $Y \approx 92\% \sim 100\%$)**：首頁、求籤、奉茶優惠、信士中心

---

### 2. 頁面二：參拜足跡總覽列表頁（Route Index）
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `<` 返回鍵 ($X \approx 5\% \sim 12\%$) ➔ 回 LINE Bye 首頁
  * 標題「參拜足跡」($X \approx 35\% \sim 65\%$) ➔ 置中
  * `X` 關閉鍵 ($X \approx 88\% \sim 95\%$) ➔ 調用 `liff.closeWindow()` 回 LINE 聊天室
* **熱門推薦路線橫幅 (Hero Route Card: $Y \approx 10\% \sim 28\%$)**：
  * `[HOT]` 標籤 + 「熱門路線 - 白沙屯繞境」+ `more >` 按鈕
