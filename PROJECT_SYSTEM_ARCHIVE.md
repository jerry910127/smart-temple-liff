...
* **頂部導航列 (Header: $Y \approx 0\% \sim 8\%$)**：
  * `LINE BYE` 品牌標題 ($X \approx 5\% \sim 30\%, Y \approx 2\% \sim 7\%$)：`LINE` 為 `#2B1810`（沉穩檀木黑）；`BYE` 採用純手繪原創**「月牙聖杯（筊）」**造型向量底牌（古法香檳金棕 `#C99F72` 漸層，帶有立體脊線光澤），中央鐫刻 `#7A0C16` 深緋紅文字，點擊帶有微傾互動；右側搭配金棕光澤 `MINI` 徽章
